...
int main()
{
    ...
    tat2.open("pri_non_pre_tat2.csv", fstream::out);
    rt2.open("pri_non_pre_rt2.csv", fstream::out);
    wt2.open("pri_non_pre_wt2.csv", fstream::out);
    ct2.open("pri_non_pre_ct2.csv", fstream::out);
    for (int i = 0; i < 100; ++i)
    {
        info.clear();
        ...
        { //use higher number as high priority
            int current_time = 0;
            priority_queue<process, vector<process>, CompareProcessMax> maxHeap;
            sort(info.begin(), info.end(), cmp_min);
            int index_in_array = 0;
            maxHeap.push(info[index_in_array]);
            current_time = info[index_in_array].AT;
            index_in_array++;
            vector<process> ans;
